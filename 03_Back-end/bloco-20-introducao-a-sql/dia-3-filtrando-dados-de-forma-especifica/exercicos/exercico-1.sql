...
SELECT first_name, store_id active FROM sakila.customer WHERE `active` <> 1 AND store_id = 2 ORDER BY first_name;
...
WHERE replacement_cost >= 18 AND rating = "NC-17" ORDER BY replacement_cost DESC, title LIMIT 100;

-- Quantos clientes estão ativos e na loja 1? As informações podem ser encontradas na tabela customer
SELECT COUNT(active) FROM sakila.customer WHERE store_id = 1 AND `active` IS true;
SELECT COUNT(active) FROM sakila.customer WHERE store_id = 1 AND `active` = 1;
...
SELECT * FROM sakila.customer WHERE store_id = 1 AND NOT `active` IS true;
SELECT * FROM sakila.customer WHERE store_id = 1 AND  `active` IS false;
SELECT * FROM sakila.customer WHERE store_id = 1 AND  `active` = 0;
...
WHERE rating = "NC-17" ORDER BY title ASC LIMIT 50;
